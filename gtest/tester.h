...
#define CHECK_UV(CMP, VAL, TYPE, REF)                       \
    CHECK_REG_##CMP(VAL);                                   \
...
#define CHECK_REG(K1, K2, REG, FROM, ...)                   \
    ...
    {                                                       \
        match |= true;                                      \
        CHECK_UV(__VA_ARGS__);                              \
        EXPECT_##FROM(nullptr, info->from);                 \
    }
...
protected:
    ...
    virtual bool check_reg(const regvm_reg_info* info);
    virtual bool check_var(const regvm_var_info* info);
